...
class BlogOne extends Component {
  ...
  render() {
    ...
    //BlogOne loop start
    const blogonedata = this.props.loading
      ? 'LOADING...'
      : events.map((event, index) => (
          <div className="col-md-6 col-lg-6" key={index}>
            <div className="blog-item">
              <Link to={`/blog-details/${event._id}`} className="blog-img">
                <img src={`http://localhost:4000/${event.image}`} alt="blog-one" />
              </Link>
              <div className="blog-info">
                <div className="date-box">
                  {this.extractDay(event.dateStart)}{' '}
                  <span className="month">{this.extractMonth(event.dateStart)}</span>
                </div>
                  ...
                  <div className="post-meta">
                    <ul>
                      <li>
                        ...
                        <Link to={event.url}>{event.user.username}</Link>
                      </li>
                      <li>
                        <Icofont icon="icofont-speech-comments" /> Comments:{' '}
                        <Link to={event.url}>{3}</Link>
                      </li>
                      <li>
                        <Icofont icon="icofont-tags" /> Tags:{' '}
                        <Link to={event.url}>{event.type}</Link>
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div className="row">
              ...
              <div className="col-lg-12 pagination-area text-center">
                <ul className="pagination">
                  <li>
                    <Link to="/#0">
                      <Icofont icon="icofont-simple-left" /> Prev
                    </Link>
                  </li>
                  <li className="active">
                    <Link to="/#0">1</Link>
                  ...
                  <li>
                    <Link to="/#0">
                      Next <Icofont icon="icofont-simple-right" />
                    </Link>
                  </li>
                </ul>
              </div>
